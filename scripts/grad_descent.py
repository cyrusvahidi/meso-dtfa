...
from meso_dtfa.loss import MultiScaleSpectralLoss, TimeFrequencyScatteringLoss, DistanceLoss
...
def run_gradient_descent(loss_type="jtfs", time_shift=10):
    ...
    jtfs_kwargs = {'shape': (n_input,), 'Q': (8,2), 'J': 12, 'J_fr': 5, 'F': 0, 'Q_fr': 2, 'format': 'time'}
    ...
    for iter in range(iters):
        ...
        grads.append(grad)
        print(f" update | iter {iter}| lr {lr} | AM pred={float(am):.3f}, target={float(theta_target[0]):.3f} | FM pred={float(fm):.3f}, target={float(theta_target[1]):.3f}, ploss={float(ploss):.3f}")
    ...
    plt.title("Gradient descent with {} loss time shifted by 2**{} samples".format(loss_type, time_shift))

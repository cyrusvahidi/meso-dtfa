...
def plot_spec(y, hop_length=256, n_fft=4096, sr=2**13):
    ...
    librosa.display.specshow(S_db, sr=sr, x_axis='time', y_axis='log', ax=ax, cmap="magma_r")
...
def plot_scalogram(Sx, S, sr=22050):
    ...
    plt.ylim(0, sr // 2)
    # plt.yticks([100, 200, 400, 800, 1600, 3200, 6400])
...
def plot_jtfs(Sx, S, sr=22050):
    ...
    plt.figure(figsize=(8, 4))
    librosa.display.specshow(Sx[1:,:], sr=sr,
        x_coords=x_coords, x_axis="time",
        cmap="magma")
    plt.xlabel("Time (seconds)")
    plt.ylabel("Frequency (Hz)")
...
def plot_jtfs_paths(jtfs, Sx):
    ...
    fig, axs = plt.subplots(len(xi_frs), len(xis), 
    ...
    for y in range(len(xi_frs)):
        for x in range(len(xis)):
            ax = axs[y, x]
            cmesh = librosa.display.specshow(paths[y, x], 
                                    sr=sr, x_axis="time", cmap="magma",
                                    ax=ax) 
    ...
    for i, ax in enumerate(axs[:, 0]):
        ...
        ax.text(0, 0, f'{xi_frs[i]:.2f}', 
                transform=ax.transAxes + trans,
                rotation=90,
                fontsize='large', ha='center', va='center', fontfamily='serif', weight='bold')
    for i, ax in enumerate(axs[-1, :]):
        trans = mtransforms.ScaledTranslation(0.75, -0.25, fig.dpi_scale_trans)
        ax.text(0.0, 0, f'{xis[i]:.4f}', 
                transform=ax.transAxes + trans,
                fontsize='large', ha='center', va='center', fontfamily='serif', weight='bold')

    fig.text(0.5, 0.1, 'temporal modulation (Hz)', fontsize='large', ha='center', va='center', weight='bold')
    fig.text(0.1, 0.5, 'frequential modulation (cycles/octave)', fontsize='large', weight='bold', ha='center', va='center', rotation='vertical')
    return paths